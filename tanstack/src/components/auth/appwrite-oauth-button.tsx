...
import { startAppwriteOAuth } from '@/lib/oauth'

function AppwriteMark(props: ComponentProps<'svg'>) {
  return (
    <svg
...
type AppwriteOAuthButtonProps = {
  mode?: 'signin' | 'signup'
  label?: string
}

export function AppwriteOAuthButton({
  mode = 'signin',
  label = mode === 'signup' ? 'Sign up with Appwrite' : 'Sign in with Appwrite',
}: AppwriteOAuthButtonProps) {
  const [isLoading, setIsLoading] = useState(false)

  const handleClick = () => {
    setIsLoading(true)
    try {
      startAppwriteOAuth(mode)
    } catch (err) {
      setIsLoading(false)
      throw err
    }
  ...
  return (
    ...
    >
      {isLoading ? (
        ...
        <AppwriteMark data-icon="inline-start" />
      )}
      {label}
    </Button>
  )
